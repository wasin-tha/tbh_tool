...
- **Secrets ที่ต้องตั้งในหน้า GitHub** (ผมตั้งให้ไม่ได้): `STEAM_USER`+`STEAM_PASS` (auto-login), `STEAM_COOKIE` (optional, auto-เขียนกลับ), `GH_PAT` (สิทธิ์ Secrets:write — ให้ write cookie กลับ), `DISCORD_WEBHOOK`
- **★ auto cookie refresh** (`steam_login.py`): cookie `steamLoginSecure` หมด → `fetch_prices.py` เรียก `steam_login.refresh()` ดึง cookie สดเขียน `steam_cookie.txt` แล้วลองยืนยันสกุลเงินใหม่. **ใช้ flow ใหม่ `IAuthenticationService`** (GetPasswordRSAPublicKey → BeginAuthSessionViaCredentials → PollAuthSessionStatus → `steamLoginSecure = steamid%7C%7Caccess_token`) — ไม่ใช้ `WebAuth` ของ lib (เก่า, endpoint `dologin` ตายแล้ว) reuse แค่ `steam.core.crypto` (`rsa_publickey`/`pkcs1v15_encrypt`) เข้ารหัสรหัสผ่าน จึงยังต้อง `pip install steam` (relogin ครั้งเดียว/run; ไม่งั้น exit 2 ตามเดิม). ⚠ บัญชี **ต้องปิด Steam Guard ทั้ง mobile+email** ไม่งั้น login ค้างรอ code. Workflow: หลัง relogin มี step `gh secret set STEAM_COOKIE` เขียน cookie สดกลับ (ต้องมี `GH_PAT` = fine-grained PAT สิทธิ์ Secrets:write; step มี `if: success()` จึงเขียนกลับเฉพาะรอบที่ run สำเร็จ) → รอบถัดไป reuse จนหมดอายุ (login ~วันละครั้ง แทนทุกรัน). ไม่มี `GH_PAT` ก็ยังทำงาน แค่ login บ่อยขึ้น
  ...
  - ✅ ทดสอบจริงผ่านแล้ว (2026-06-14): local + Action เขียว, cookie สดคืน ฿ จริง, relogin/write-back/Discord-gate ทำงานถูก
...
- มี `update_prices.bat` (local, double-click, **gitignore แล้ว** — มี `STEAM_USER`/`STEAM_PASS` plaintext + path เฉพาะเครื่อง ห้ามหลุด) = ตัวสำรองของ Action: set creds → fetch_prices → gen → `git add index.html prices.json` → `git pull --rebase` → push (mirror Action เป๊ะ) — ใช้ **full path python** (เลี่ยง Store stub) + ต้องเป็น **CRLF**
- action เวอร์ชัน: `actions/checkout@v5` + `actions/setup-python@v6` (Node 24)
