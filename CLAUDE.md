...
- **★ auto cookie refresh** (`steam_login.py`): cookie `steamLoginSecure` หมด → `fetch_prices.py` เรียก `steam_login.refresh()` ดึง cookie สดเขียน `steam_cookie.txt` แล้วลองยืนยันสกุลเงินใหม่. **ใช้ flow ใหม่ `IAuthenticationService`** (GetPasswordRSAPublicKey → BeginAuthSessionViaCredentials → PollAuthSessionStatus → `steamLoginSecure = steamid%7C%7Caccess_token`) — ไม่ใช้ `WebAuth` ของ lib (เก่า, endpoint `dologin` ตายแล้ว) reuse แค่ `steam.core.crypto` (`rsa_publickey`/`pkcs1v15_encrypt`) เข้ารหัสรหัสผ่าน จึงยังต้อง `pip install steam` (relogin ครั้งเดียว/run; ไม่งั้น exit 2 ตามเดิม). ⚠ บัญชี **ต้องปิด Steam Guard ทั้ง mobile+email** ไม่งั้น login ค้างรอ code. Workflow: หลัง relogin มี step `gh secret set STEAM_COOKIE` เขียน cookie สดกลับ (ต้องมี `GH_PAT`) → รอบถัดไป reuse จนหมดอายุ (login ~วันละครั้ง แทนทุกรัน). ไม่มี `GH_PAT` ก็ยังทำงาน แค่ login บ่อยขึ้น
