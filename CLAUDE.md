...
- **UI**: drag/drop หรือเลือกไฟล์ → cache base64 ใน `localStorage['tbh_save_b64']` (reload อ่านคืนเอง) → sub-tabs All/Stash/Inventory/Trade Ship/Heroes + filter grade(multi, **มี grade-dot สีเหมือน Material**)/type/tradable + search + **sort (custom `.fdd` dropdown `data-fdd="stsort"` ใช้ `toggleFdd` ร่วม; default=`unit` ราคาต่อชิ้น, จำใน `localStorage['tbh_stash_sort']`; `stRenderSort`/`stPickSort`/`ST_SORT_OPTS`)** + panel รายละเอียด (วัสดุที่ใส่ + Steam link) + หน้า Heroes (การ์ด slot อุปกรณ์ + gear value ต่อฮีโร่; **portrait อยู่ใน header คู่ชื่ออาชีพ `.st-hero-id`, body เป็น 2 คอลัมน์ slot**)
...
- **เทส**: Playwright serve ผ่าน `http.server` (**ต้อง http/localhost — `crypto.subtle` ต้อง secure context, `file://` ใช้ไม่ได้**) → คลิกแท็บ stash → `set_input_files('#st-file', SaveFile_Live.es3)` → เช็ค `#st-nw`. ยืนยันกับเซฟจริง: NW ฿418.58 ≈ tbhindex ฿415.63, จำนวน All 421/Stash 320/Inv 101/Ship 0/Heroes 5 **ตรงเป๊ะ**. `browser.new_page()` = context ใหม่ (localStorage แยก) → ทดสอบ cache reload ต้อง `page.reload()` context เดิม
- **★ กราฟราคา history (ยังไม่ทำ — reference เผื่อทำต่อ)**: tbhindex เก็บเองใน backend (`POST api.tbhindex.com/api/items/history/batch` body `{names:[hash]}` → `{hash:[{timestamp,median_price,volume}]}` เป็น **USD**; ราคาสดที่ `GET /api/live/items` → `{items:[{market_hash_name,lowest_price,median_price,volume}]}` USD). hash name = material→`name`, gear→`"{Name} ({Grade} title-case) A"`. **แนะนำของเรา**: เก็บ snapshot รายวันเองใส่ `history.json` (Action มี fetch อยู่แล้ว) — ใช้ราคา ฿ ของเรา; backfill ไม่ได้ กราฟจะเริ่มบางแล้วค่อยเต็ม. Steam ไม่ให้ price history ฟรี (ต้อง login cookie ต่อ listing)
